...
echo "Try to write to remote database..."
...
cp "$sqlBackup" \
    "$app_folder/backup/${mysql_database_local}_$timeScriptStart.sql"
removeRemoteFile "$remote_root/$sqlBackup"
removeRemoteFile "$remote_root/$sqlScript"
rm "$sqlBackup"
rm "$sqlScript"
...
if [ ! -f "$zip_file" ]; then
    ...
    echo "Upload and Extract $zip_file to remote server..."
    ...
    if zipinfo "$difzip" > /dev/null; then
        ...
        cp "$zip_file" \
            "$app_folder/backup/${zip_basename%.*}_$timeScriptStart.zip"
    ...
    cd "$local_root"
    echo "Eventually remove remote files if missing in local folder..."
